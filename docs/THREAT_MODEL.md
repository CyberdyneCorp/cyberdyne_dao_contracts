...
| 24 | CostRegistry payment-token migration to a bad/decimals-mismatched token | `setPaymentToken` is vote-gated (`UPDATE_PAYMENT_TOKEN_PERMISSION`); `costUsdc` is raw token units, so a decimals change must be paired with `updateEntry` in the same proposal (documented). | `CostRegistryPlugin.unit.test.ts` setPaymentToken cases | Per-migration review |
