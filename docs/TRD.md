...
1. **AAVE v4 launch date.** Adapter shape mitigates, but if v4's interface diverges significantly from v3 (e.g., the announced "GHO-native" liquidity layer), the `IAaveAdapter` interface may need a v2. Tracking: `aave/aave-v4` repo.
...
4. **Gas ceiling on payroll batch.** Up to ~100 recipients tested; beyond that we'd need a paginated crank (`executePayroll(uint256 startIdx, uint256 count)` with idempotency per page per period).
