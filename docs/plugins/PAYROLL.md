...
| `cyclomatic-complexity` on `_runPayroll` | `_runPayroll` | Accepted. Function aggregates the page/full crank, per-recipient failure tolerance, bounty bookkeeping, and the cursor advance — splitting would obscure the effects-before-interactions ordering the `reentrancy-events` waiver depends on. |
