...
- [~] 🟡 **Pre-submit quote previews for V3 + V4 mint.** V3 shipped (the pool-state preview above shows current tick + in/below/above-range verdict). V4 still pending — needs a v4 QuoterV2-equivalent read; tracked under the V4 `previewModifyLiquidities` quote-helper stretch item.
