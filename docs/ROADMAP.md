...
- Unit tests (`test/plugins/payroll/PayrollPlugin.unit.test.ts`):
  ...
  - Mandatory salary transfers (audit H-01): a reverting/false-returning recipient reverts the whole crank (`allowFailureMap = 0`); the period stays open to retry. (Superseded the original "per-recipient failure tolerance" — see Phase 10 remediation.)
...
- [x] ✅ **ETH-in helper (frontend).** The plugin stays ERC20-only (it never holds a payable surface); instead the frontend exposes `wethDepositAction(cfg, amountWei)` and the V3 mint form has an optional "wrap ETH" field that prepends a `WETH.deposit{value}` action to the mint batch, so the DAO wraps ETH → WETH and mints atomically in one proposal. Same composition works for V4 LP / increase.

### UniswapV4Plugin

- [x] ✅ **Live V4 LP fork test.** Two new mainnet-only fork tests in `UniswapV4Plugin.fork.test.ts`: (1) mints a REAL USDC/WETH V4 position (fee 3000, tickSpacing 60, no hooks, range straddling the live tick ~199951) via `modifyLiquidities` against the canonical v4 PositionManager `0xbD21…ee9e`, asserting the NFT is owned by the DAO + zero plugin custody + zero residual DAO→Permit2 allowance; (2) asserts `MintRecipientMustBeDao` reverts when the encoded owner ≠ DAO. Deadlines are derived from the fork's block clock (not wall-clock) since the shared anvil fork's timestamp drifts forward from payroll time-travel tests. The V4-native *swap* encoding stays deferred to P5 e2e (the swap path is already proven via the V3 Universal Router command).
...
- [x] ✅ **V4 pool-state quote helper.** `frontend/src/lib/v4Quote.ts`: `readV4PoolState(cfg, provider, poolKey)` derives the v4 PoolId (`keccak256(abi.encode(PoolKey))`), reads `getSlot0` + `getLiquidity` via the periphery `StateView` lens, and returns `{poolId, sqrtPriceX96, tick, liquidity, rawPriceToken1PerToken0, inRange}`. The V4 mint form gains a "Quote pool" button with the same in/below/above-range verdict as V3. **Feature-flagged** on a per-chain `UNISWAP_V4_STATE_VIEW` address (like the subgraph/IPFS integrations): the button is hidden until the address is added to the address book — deliberately not hardcoding an unverified StateView address. Adding the verified per-chain StateView address to `addresses.json` is the one remaining step to turn it on.

### PayrollPlugin

- [x] ✅ **Keeper bounty.** Vote-gated `setKeeperBounty(token, perCrank, maxPerPeriod)` (new `UPDATE_BOUNTY_PERMISSION`) pays `msg.sender` an ETH-or-ERC20 bounty out of the DAO on each successful crank. Per-period rolling cap is share by paginated calls in the same month. Bounty is the only failable leg of the `dao.execute` batch (salaries are mandatory — see Phase 10 / audit H-01), so a bounty failure doesn't block recipients, and `KeeperBountyPaid` is emitted only when the bounty actually paid. Default = disabled (`perCrank = 0`). Closes TRD §16 #3. Setup grant-list grows 3 → 4. 6 new unit tests.
...
- [x] ✅ **Vote-gated `setMaxRecipients(uint256)`.** `MAX_RECIPIENTS()` is now a storage value (default 300) gated by `MANAGE_PAYROLL`, bounded by a new hard `MAX_RECIPIENTS_CEILING = 1000` and never below the live slot count (`MaxRecipientsOutOfRange`). Storage gap 40→39; layout doc updated. 6 unit tests + a fuzz action in the invariant handler (the `recipientCountBounded` invariant holds across cap changes).
- [x] ✅ **`previewForcePayPeriodActions(uint256 period)`.** Vote-gated recovery for a skipped month, shipped as a `preview…Actions` view (NOT a wrapper that calls `dao.execute` itself — that would nest `dao.execute` and trip OSx's `nonReentrant`, the same reason AAVE/Uniswap fund ops use the preview path). Returns the DAO→payee transfer batch for every active recipient; `period` must be strictly between `lastPayoutPeriod` and now and ≤ `MAX_FORCE_BACK_MONTHS = 12` back (guards revert at build/sim time). 5 unit tests (incl. executing the batch through the DAO mock).

### CostRegistryPlugin

- [x] ✅ **`setPaymentToken(address)` vote-gated migration.** Plugin now exposes `setPaymentToken(address)` gated by new permission `UPDATE_PAYMENT_TOKEN_PERMISSION`; emits `PaymentTokenUpdated(previous, current)`. Setup install grant-list grows 3 → 4. Switching to a token with different decimals is now **rejected** (`PaymentTokenDecimalsMismatch`) — see Phase 10 / audit CR-M-02; only a same-decimals swap is allowed.
...
- [x] ✅ **`processAllDue()`** — keeper-friendlier crank that pays the first `MAX_PER_PAGE` due entries from index 0 (genuinely "all due" for any registry ≤ one page; larger registries still paginate via `processDue`). Refactored the shared body into a private `_processDue(offset, limit)`. Unit test added.
- [x] ✅ **Vote-gated `setMaxEntries(uint256)`.** Same shape as the Payroll item: `MAX_ENTRIES()` is now storage (default 300) gated by `MANAGE_COSTS`, bounded by `MAX_ENTRIES_CEILING = 1000`, never below the live slot count (`MaxEntriesOutOfRange`). Storage gap 48→47; layout doc updated. 6 unit tests + invariant fuzz action.

### Frontend (toy SvelteKit dApp)

- [x] ✅ **Subgraph wired (feature-flagged).** New `frontend/src/lib/subgraph.ts` — a dependency-free `fetch`-based GraphQL client (no `@urql`) gated on `PUBLIC_SUBGRAPH_URL`. Swaps page queries the subgraph when configured (full history) and falls back to the RPC block-scan otherwise, with a source badge. Costs page gains a subgraph-only payment-history table (`CostPaid`). Typed helpers `fetchSwaps` / `fetchCostPayments` / `fetchV3Positions`. `.env.example` documents the var. Production build green.
- [x] ✅ **Pre-submit quote preview (V3).** The V3 mint form's "Quote pool" button shows current tick + in/below/above-range verdict (see UniswapV3Plugin above). The V4 equivalent is tracked as the single open 🔵 below (`previewModifyLiquidities` quote helper) — it needs a v4 StateView address in the per-chain address book, deferred as a stretch.
- [x] ✅ **AAVE health-factor banner.** Prominent color-coded banner at the top of the lending page: green ≥ 1.5 (healthy), yellow 1.0–1.5 (caution), red < 1.0 (liquidatable), gray ∞ (no debt). Plain-English blurb explains the risk per tier. (Post-borrow projection deferred — it needs a price-feed integration; the proposal-execution simulation item will cover that more cleanly.)
- [x] ✅ **Proposal-execution simulation.** New `simulateProposalExecution(cfg, provider, actions)` helper runs `dao.callStatic.execute(callId, actions, 0)` with `from = TokenVoting plugin address` (which has `EXECUTE_PERMISSION`). Returns `{ok: true}` or `{ok: false, reason}` with the decoded revert reason. Proposals table now has a per-row "Sim" column that exposes pre-execute reverts (bad slippage, expired deadline, insufficient balance, `MintRecipientMustBeDao`, …) so voters see them BEFORE the vote ends. Results cached per-proposal; reset on refresh.
...
- [x] ✅ **IPFS proposal-metadata pinning.** New `frontend/src/lib/ipfs.ts` — feature-flagged on `PUBLIC_PINATA_JWT`. When set, `proposeActions` pins a structured metadata doc (schema `cyberdyne-proposal/1`: title/description/actions/createdAt) and stores the `ipfs://CID` as the proposal's on-chain metadata; otherwise it stores the summary inline as UTF-8 (toy-frontend default). The pinned doc matches the canonical schema in `docs/PROPOSAL_METADATA.md` exactly (`title`/`description`/`encodedActions[].humanReadable`). Pin failure is non-fatal (falls back to inline). The proposals list resolves `ipfs://` metadata back to a human title via a public gateway (`PUBLIC_IPFS_GATEWAY`, default ipfs.io), best-effort. Closes the slide-12 claim gap.
...
- [x] ✅ **`BorrowHealthCondition` contract** (TRD §16 #1) — `src/plugins/aave/conditions/BorrowHealthCondition.sol`, an OSx `PermissionCondition` enforcing a min health factor (18-dec, governance-settable via `setMinHealthFactor` gated to the DAO; fail-closed). Two surfaces for the two borrow paths: `isGranted` projects the post-borrow HF to gate the **direct** `borrow()` path via `grantWithCondition` (operator setups); `assertHealthFactor(dao)` is appended as the final in-batch action of a **governance** borrow proposal (the preview path bypasses plugin auth, so a condition alone can't see it) and reverts the batch if HF < floor. 15 unit tests + 4 mainnet-fork tests (incl. projection ≈ AAVE's own HF within ~1% on a live position). Deploy via `scripts/DeployBorrowHealthCondition.s.sol`; see `docs/plugins/AAVE.md §5`.
